...
func Example_notFoundError() {
	bookID := "book-123"
	err := errors.ErrNotFound.WithDetails("book_id", bookID)
...
func Example_httpStatus() {
	...
	err2 := errors.ErrValidation
	status2 := errors.GetHTTPStatus(err2)
	fmt.Println(status2)
...
func Example_errorComparison() {
	...
	err := errors.ErrValidation.WithDetails("field", "email")

	// Compare with base error
	isValidation := errors.Is(err, errors.ErrValidation)
	isNotFound := errors.Is(err, errors.ErrNotFound)
...
func Example_domainSpecificError() {
	// Book not found
	err := errors.ErrBookNotFound.WithDetails("book_id", "book-456")
	fmt.Println(err.Code)
	fmt.Println(err.Message)

	// Invalid ISBN
	err2 := errors.ErrInvalidISBN.
