...
// GetAuthToken retrieves a cached OAuth token or fetches a new one from edomain.kz.
//
...
// until it expires, minimizing API calls to the payment provider.
func (g *Gateway) GetAuthToken(ctx context.Context) (string, error) {
	...
	logger.Info("fetching new auth token from edomain.kz")

	// Prepare OAuth request with correct scope from edomain.kz docs
