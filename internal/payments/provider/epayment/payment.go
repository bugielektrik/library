...
//
// This method queries the edomain.kz provider to retrieve the current status
...
func (g *Gateway) CheckPaymentStatus(ctx context.Context, invoiceID string) (*domain.GatewayStatusResponse, error) {
	...
	// Map epayment response to domain provider response
	return &domain.GatewayStatusResponse{
		ResultCode:    statusResp.ResultCode,
		ResultMessage: statusResp.ResultMessage,
		Transaction: domain.GatewayTransactionDetails{
...
// RefundPayment processes a refund for a completed domain.
//
...
func (g *Gateway) ChargeCard(ctx context.Context, req *domain.CardChargeRequest) (*domain.CardChargeResponse, error) {
	...
	return &domain.CardChargeResponse{
		ID:            paymentResp.ID,
