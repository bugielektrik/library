...
// account depending on the payment provider and bank processing times.
...
// never stored; only the provider-provided token is used.
//
// Validation rules:
...
//   - CVV may be required depending on provider configuration
//
// Example JSON:
//
//	{
...
// or asynchronously (status updated via callback), depending on provider configuration.
