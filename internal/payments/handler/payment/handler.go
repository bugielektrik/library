...
//	POST   /callback              → handleCallback (payment_callback.go) - PUBLIC endpoint for provider
...
//   - /callback is called by payment provider, validated by signature/token
...
func (h *PaymentHandler) Routes() chi.Router {
	...
	// Webhook endpoint (public, called by payment provider)
