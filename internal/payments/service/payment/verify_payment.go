...
// Execute verifies the status of a payment by checking with the provider.
func (uc *VerifyPaymentUseCase) Execute(ctx context.Context, req VerifyPaymentRequest) (VerifyPaymentResponse, error) {
	...
	// If payment is not in final state, check status with provider
	if isPaymentUpdatable(paymentEntity.Status) {
		logger.Info("checking payment status with provider", zap.String("invoice_id", paymentEntity.InvoiceID))

		// Call provider API to check status
		statusResp, err := uc.paymentGateway.CheckPaymentStatus(ctx, paymentEntity.InvoiceID)
		if err != nil {
			logger.Warn("failed to check payment status with provider", zap.Error(err))
			// Don't fail the request, just return current status
		} else {
			logger.Info("provider status check successful",
...
// updatePaymentFromGatewayResponse updates payment based on provider status response.
