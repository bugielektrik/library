// Package paymentops implements use cases for payment processing service.
//
...
//   - InitiatePaymentUseCase: Starts payment process with provider
//   - ProcessPaymentCallbackUseCase: Handles payment provider callbacks
...
//   - domain.Gateway: External payment provider adapter (edomain.kz)
//   - domain.Repository: For updating member subscription status
//
// Example usage:
//
//	initiateUC := paymentops.NewInitiatePaymentUseCase(paymentRepo, provider, memberRepo)
...
//
// Payment flow:
//  1. InitiatePayment: Create payment record, get provider URL
...
//
// Security:
//   - Payment provider signatures verified (SHA-256 HMAC)
