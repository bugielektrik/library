...
import (
	...
	"encoding/json"
	"fmt"
	"library-service/internal/pkg/sqlutil"
...
// ReceiptRepository implements domain.ReceiptRepository for PostgreSQL
type ReceiptRepository struct {
	db *sqlx.DB
}

// Compile-time check that ReceiptRepository implements domain.ReceiptRepository
var _ domain.ReceiptRepository = (*ReceiptRepository)(nil)
...
func (r *ReceiptRepository) GetByID(ctx context.Context, id string) (domain.Receipt, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return domain.Receipt{}, fmt.Errorf("receipt not found: %s", id)
		}
		return domain.Receipt{}, fmt.Errorf("failed to get receipt: %w", err)
	}
...
func (r *ReceiptRepository) GetByPaymentID(ctx context.Context, paymentID string) (domain.Receipt, error) {
	query := `
		SELECT id, payment_id, receipt_number, member_id, amount, currency,
		       payment_type, payment_method, transaction_id, payment_date, receipt_date,
		       status, description, member_name, member_email, card_mask,
	...
	var row receiptRow
	err := r.db.GetContext(ctx, &row, query, paymentID)
	if err != nil {
		if err == sql.ErrNoRows {
			return domain.Receipt{}, fmt.Errorf("receipt not found for payment: %s", paymentID)
		}
		return domain.Receipt{}, fmt.Errorf("failed to get receipt: %w", err)
	}
...
func (r *ReceiptRepository) GetByReceiptNumber(ctx context.Context, receiptNumber string) (domain.Receipt, error) {
	query := `
		SELECT id, payment_id, receipt_number, member_id, amount, currency,
		       payment_type, payment_method, transaction_id, payment_date, receipt_date,
		       status, description, member_name, member_email, card_mask,
	...
	var row receiptRow
	err := r.db.GetContext(ctx, &row, query, receiptNumber)
	if err != nil {
		if err == sql.ErrNoRows {
			return domain.Receipt{}, fmt.Errorf("receipt not found: %s", receiptNumber)
		}
		return domain.Receipt{}, fmt.Errorf("failed to get receipt: %w", err)
	}
...
func (r *ReceiptRepository) ListByMemberID(ctx context.Context, memberID string) ([]domain.Receipt, error) {
	query := `
		SELECT id, payment_id, receipt_number, member_id, amount, currency,
		       payment_type, payment_method, transaction_id, payment_date, receipt_date,
		       status, description, member_name, member_email, card_mask,
	...
	var rows []receiptRow
	err := r.db.SelectContext(ctx, &rows, query, memberID)
	if err != nil {
		return nil, fmt.Errorf("failed to list receipts: %w", err)
	}

	receipts := make([]domain.Receipt, len(rows))
...
func (r *ReceiptRepository) rowToReceipt(row receiptRow) (domain.Receipt, error) {
	// Unmarshal items
	var items []domain.ReceiptItem
	if len(row.Items) > 0 {
		if err := json.Unmarshal(row.Items, &items); err != nil {
			return domain.Receipt{}, fmt.Errorf("failed to unmarshal items: %w", err)
		}
	}
