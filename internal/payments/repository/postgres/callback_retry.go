...
// CallbackRetryRepository implements domain.CallbackRetryRepository for PostgreSQL
type CallbackRetryRepository struct {
	postgres.BaseRepository[domain.CallbackRetry]
}

// Compile-time check that CallbackRetryRepository implements domain.CallbackRetryRepository
var _ domain.CallbackRetryRepository = (*CallbackRetryRepository)(nil)
...
func NewCallbackRetryRepository(db *sqlx.DB) *CallbackRetryRepository {
	return &CallbackRetryRepository{
		BaseRepository: postgres.NewBaseRepository[domain.CallbackRetry](db, "callback_retries"),
	}
}

// Create inserts a new callback retry record
func (r *CallbackRetryRepository) Create(callbackRetry *domain.CallbackRetry) error {
	query := `
...
func (r *CallbackRetryRepository) GetByID(id string) (*domain.CallbackRetry, error) {
	query := `
	...
	var callbackRetry domain.CallbackRetry
...
func (r *CallbackRetryRepository) GetPendingRetries(limit int) ([]*domain.CallbackRetry, error) {
	query := `
		SELECT id, payment_id, callback_data, retry_count, max_retries,
		       last_error, next_retry_at, status, created_at, updated_at
		FROM callback_retries
	...
	var callbackRetries []*domain.CallbackRetry
	err := r.GetDB().Select(&callbackRetries, query, domain.CallbackRetryStatusPending, limit)
	if err != nil {
...
func (r *CallbackRetryRepository) Update(callbackRetry *domain.CallbackRetry) error {
	query := `
