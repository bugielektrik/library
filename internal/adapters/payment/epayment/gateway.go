...
// Timeout and expiry constants for payment gateway configuration
const (
	// DefaultHTTPTimeout is the default timeout for HTTP requests to the payment gateway.
	// This includes OAuth token requests, payment status checks, refunds, and cancellations.
	DefaultHTTPTimeout = 30 * time.Second

	// MaxRetries is the maximum number of retry attempts for failed payment operations.
	// Currently not implemented but reserved for future retry logic.
	MaxRetries = 3
)

// Config holds the configuration for epayment.kz gateway.
type Config struct {
	// OAuth configuration
	...
	PostLink    string // URL where payment gateway sends callbacks
	WidgetURL   string // URL for payment widget script
	Environment string // Environment (test/prod)
}

// Gateway represents the epayment.kz payment gateway client.
//
...
// NewGateway creates a new payment gateway instance.
//
// Parameters:
...
// GetPostLink returns the URL where payment gateway sends callbacks.
func (g *Gateway) GetPostLink() string {
	return g.config.PostLink
}

// GetBaseURL returns the base URL of the payment gateway API.
