...
func newTestGateway(baseURL, oauthURL string) *Gateway {
	config := &Config{
		...
		BaseURL:      baseURL,
		OAuthURL:     oauthURL,
		WidgetURL:    "https://test-widget.epayment.kz",
...
func TestGetAuthToken_Success(t *testing.T) {
	...
	gateway := newTestGateway("https://test-api.epayment.kz", oauthServer.URL)
...
func TestGetAuthToken_NetworkError(t *testing.T) {
	...
	gateway := newTestGateway("https://test-api.epayment.kz", serverURL)
