...
// PaymentRepository implements payment.Repository interface for PostgreSQL.
type PaymentRepository struct {
	BaseRepository[payment.Payment]
}

// NewPaymentRepository creates a new PostgreSQL payment repository.
func NewPaymentRepository(db *sqlx.DB) *PaymentRepository {
	return &PaymentRepository{
		BaseRepository: NewBaseRepository[payment.Payment](db, "payments"),
	}
}
...
func (r *PaymentRepository) GetByID(ctx context.Context, id string) (payment.Payment, error) {
	...
	if err != nil {
		...
		if err == store.ErrorNotFound {
			return payment.Payment{}, errors.ErrNotFound.WithDetails("payment_id", id)
		}
		return payment.Payment{}, fmt.Errorf("failed to get payment: %w", err)
	}
...
func (r *PaymentRepository) GetByInvoiceID(ctx context.Context, invoiceID string) (payment.Payment, error) {
	query := `
		SELECT
			id, invoice_id, member_id, amount, currency, status, payment_method, payment_type,
			related_entity_id, gateway_transaction_id, gateway_response, card_mask, approval_code,
	...
	if err != nil {
		err = HandleSQLError(err)
		if err == store.ErrorNotFound {
			return payment.Payment{}, errors.ErrNotFound.WithDetails("invoice_id", invoiceID)
		}
		return payment.Payment{}, fmt.Errorf("failed to get payment by invoice ID: %w", err)
	}
...
func (r *PaymentRepository) Update(ctx context.Context, id string, payment payment.Payment) error {
	query := `
		UPDATE payments SET
			status = $2,
			payment_method = $3,
...
func (r *PaymentRepository) ListByMemberID(ctx context.Context, memberID string) ([]payment.Payment, error) {
	query := `
		SELECT
			id, invoice_id, member_id, amount, currency, status, payment_method, payment_type,
			related_entity_id, gateway_transaction_id, gateway_response, card_mask, approval_code,
...
func (r *PaymentRepository) ListByStatus(ctx context.Context, status payment.Status) ([]payment.Payment, error) {
	query := `
		SELECT
			id, invoice_id, member_id, amount, currency, status, payment_method, payment_type,
			related_entity_id, gateway_transaction_id, gateway_response, card_mask, approval_code,
...
func (r *PaymentRepository) UpdateStatus(ctx context.Context, id string, status payment.Status) error {
	query := `
		UPDATE payments SET
			status = $2,
			updated_at = NOW()
...
func (r *PaymentRepository) ListExpired(ctx context.Context) ([]payment.Payment, error) {
	query := `
		SELECT
			id, invoice_id, member_id, amount, currency, status, payment_method, payment_type,
			related_entity_id, gateway_transaction_id, gateway_response, card_mask, approval_code,
...
func (r *PaymentRepository) ListPendingByMemberID(ctx context.Context, memberID string) ([]payment.Payment, error) {
	query := `
		SELECT
			id, invoice_id, member_id, amount, currency, status, payment_method, payment_type,
			related_entity_id, gateway_transaction_id, gateway_response, card_mask, approval_code,
