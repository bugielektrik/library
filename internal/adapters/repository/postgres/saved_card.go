...
// SavedCardRepository implements payment.SavedCardRepository interface for PostgreSQL.
type SavedCardRepository struct {
	BaseRepository[payment.SavedCard]
}
...
func NewSavedCardRepository(db *sqlx.DB) *SavedCardRepository {
	return &SavedCardRepository{
		BaseRepository: NewBaseRepository[payment.SavedCard](db, "saved_cards"),
	}
}

// Create inserts a new saved card and returns its ID.
func (r *SavedCardRepository) Create(ctx context.Context, card payment.SavedCard) (string, error) {
	query := `
...
func (r *SavedCardRepository) GetByID(ctx context.Context, id string) (payment.SavedCard, error) {
	query := `
		SELECT
	...
	var card payment.SavedCard
	err := r.GetDB().GetContext(ctx, &card, query, id)
	if err != nil {
		if err == sql.ErrNoRows {
			return payment.SavedCard{}, errors.ErrNotFound.WithDetails("card_id", id)
		}
		return payment.SavedCard{}, fmt.Errorf("failed to get saved card: %w", err)
	}
...
func (r *SavedCardRepository) GetByCardToken(ctx context.Context, cardToken string) (payment.SavedCard, error) {
	query := `
		SELECT
			id, member_id, card_token, card_mask, card_type, expiry_month, expiry_year,
			is_default, is_active, created_at, updated_at, last_used_at
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return payment.SavedCard{}, errors.ErrNotFound.WithDetails("card_token", cardToken)
		}
		return payment.SavedCard{}, fmt.Errorf("failed to get saved card by token: %w", err)
	}
...
func (r *SavedCardRepository) ListByMemberID(ctx context.Context, memberID string) ([]payment.SavedCard, error) {
	query := `
		SELECT
			id, member_id, card_token, card_mask, card_type, expiry_month, expiry_year,
			is_default, is_active, created_at, updated_at, last_used_at
...
func (r *SavedCardRepository) GetDefaultCard(ctx context.Context, memberID string) (payment.SavedCard, error) {
	query := `
		SELECT
			id, member_id, card_token, card_mask, card_type, expiry_month, expiry_year,
			is_default, is_active, created_at, updated_at, last_used_at
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return payment.SavedCard{}, errors.ErrNotFound.WithDetails("member_id", memberID)
		}
		return payment.SavedCard{}, fmt.Errorf("failed to get default card: %w", err)
	}
...
func (r *SavedCardRepository) Update(ctx context.Context, id string, card payment.SavedCard) error {
	query := `
