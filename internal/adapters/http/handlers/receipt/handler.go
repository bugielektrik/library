...
// @Produce json
// @Security BearerAuth
// @Param request body dto.GenerateReceiptRequest true "Receipt generation request"
// @Success 201 {object} dto.ReceiptResponse "Receipt generated"
...
func (h *ReceiptHandler) generateReceipt(w http.ResponseWriter, r *http.Request) {
	...
	// Decode request
	var req dto.GenerateReceiptRequest
	...
	// Execute use case
	result, err := h.useCases.Receipt.GenerateReceipt.Execute(ctx, paymentops.GenerateReceiptRequest{
	...
	fullReceipt, err := h.useCases.Receipt.GetReceipt.Execute(ctx, paymentops.GetReceiptRequest{
	...
	h.RespondJSON(w, http.StatusCreated, dto.FromReceiptEntity(fullReceipt.Receipt))
}
...
// @Success 200 {object} dto.ReceiptResponse "Receipt details"
// @Failure 401 {object} dto.ErrorResponse "Unauthorized"
...
func (h *ReceiptHandler) getReceipt(w http.ResponseWriter, r *http.Request) {
	...
	// Execute use case
	result, err := h.useCases.Receipt.GetReceipt.Execute(ctx, paymentops.GetReceiptRequest{
		ReceiptID: receiptID,
		MemberID:  memberID,
	})
	...
	h.RespondJSON(w, http.StatusOK, dto.FromReceiptEntity(result.Receipt))
}
...
// @Produce json
// @Security BearerAuth
// @Success 200 {object} dto.ListReceiptsResponse "List of receipts"
// @Failure 401 {object} dto.ErrorResponse "Unauthorized"
// @Failure 500 {object} dto.ErrorResponse "Internal server error"
// @Router /receipts [get]
func (h *ReceiptHandler) listReceipts(w http.ResponseWriter, r *http.Request) {
	...
	// Execute use case
	result, err := h.useCases.Receipt.ListReceipts.Execute(ctx, paymentops.ListReceiptsRequest{
		MemberID: memberID,
	})
	...
	h.RespondJSON(w, http.StatusOK, dto.ListReceiptsResponse{
		Receipts: dto.FromReceiptEntities(result.Receipts),
		Total:    result.Total,
	})
}
