...
// BaseHandler provides common handler functionality that can be embedded in all handlers.
//
// This eliminates duplication of respondError and respondJSON methods across handlers.
...
func (b *BaseHandler) GetMemberID(w http.ResponseWriter, r *http.Request) (string, bool) {
	...
	if !ok {
		b.RespondError(w, r, errors.ErrUnauthorized)
		return "", false
	}
...
func (b *BaseHandler) GetURLParam(w http.ResponseWriter, r *http.Request, paramName string) (string, bool) {
	value, err := httputil.GetURLParam(r, paramName)
	if err != nil {
