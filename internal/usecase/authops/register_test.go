	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(repo *mocks.MockMemberRepository) {
				...
				repo.On("Add", mock.Anything, mock.MatchedBy(func(m member.Member) bool {
					return m.Email == "newuser@example.com" &&
						m.PasswordHash != "" &&
						m.Role == member.RoleUser
				})).
...
func TestRegisterUseCase_PasswordHashing(t *testing.T) {
	...
	mockRepo.On("Add", mock.Anything, mock.MatchedBy(func(m member.Member) bool {
