...
func NewSetDefaultCardUseCase(savedCardRepo payment.SavedCardRepository) *SetDefaultCardUseCase {
...
func (uc *SetDefaultCardUseCase) Execute(ctx context.Context, req SetDefaultCardRequest) (SetDefaultCardResponse, error) {
	...
	if err != nil {
		...
		return SetDefaultCardResponse{}, errors.NotFoundWithID("card", req.CardID)
	}
	...
	if !card.CanBeUsed() {
		...
		return SetDefaultCardResponse{}, errors.NewError(errors.CodeValidation).
	...
	if err := uc.savedCardRepo.SetAsDefault(ctx, req.MemberID, req.CardID); err != nil {
		...
		return SetDefaultCardResponse{}, errors.Database("database operation", err)
	}
