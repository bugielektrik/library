...
func (uc *PayWithSavedCardUseCase) validateSavedCard(
	...
	if !savedCard.CanBeUsed() {
		...
		return payment.SavedCard{}, errors.NewError(errors.CodeValidation).
...
func (uc *PayWithSavedCardUseCase) createPaymentRecord(
	...
	paymentEntity.PaymentMethod = payment.PaymentMethodCard
	...
	if err != nil {
		...
		return payment.Payment{}, errors.Database("database operation", err)
	}
...
func (uc *PayWithSavedCardUseCase) chargeCardViaGateway(
	...
	chargeReq := &payment.CardChargeRequest{
	...
	if err != nil {
		...
		return payment.Payment{}, errors.External("payment gateway", err)
	}

	// Update payment with gateway response
	UpdatePaymentFromCardCharge(&paymentEntity, gatewayResp, uc.paymentService)
