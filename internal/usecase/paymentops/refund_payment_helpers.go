...
func validateRefundAuthorization(req RefundPaymentRequest, paymentEntity payment.Payment, logger *zap.Logger) error {
	...
	if paymentEntity.MemberID != req.MemberID {
		...
		return errors.NotFoundWithID("payment", req.PaymentID)
	}
...
func validateRefundAmount(req RefundPaymentRequest, paymentEntity payment.Payment, logger *zap.Logger) (int64, bool, error) {
	...
	if requestedAmount <= 0 {
		...
		return 0, false, errors.NewError(errors.CodeValidation).
...
func validateRefundEligibility(paymentEntity payment.Payment, paymentService *payment.Service, logger *zap.Logger) error {
	...
	if err := paymentService.ValidateStatusTransition(paymentEntity.Status, payment.StatusRefunded); err != nil {
