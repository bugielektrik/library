...
//
// See Also:
//   - Domain service: internal/reservations/domain/service.go (business rules)
//   - Similar pattern: internal/books/operations/create_book.go (simple CRUD)
//   - HTTP handler: internal/reservations/http/handler.go
//   - Repository: internal/reservations/repository/reservation.go
//   - ADR: .claude/adr/002-clean-architecture-boundaries.md (layer dependencies)
//   - Test: internal/reservations/operations/create_reservation_test.go
...
func (uc *CreateReservationUseCase) Execute(ctx context.Context, req CreateReservationRequest) (CreateReservationResponse, error) {
	...
	reservationEntity := reservationdomain.New(reservationdomain.Request{
		BookID:   req.BookID,
