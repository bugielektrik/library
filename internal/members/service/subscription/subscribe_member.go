...
func NewSubscribeMemberUseCase(memberRepo domain.Repository, memberService *domain.Service) *SubscribeMemberUseCase {
...
func (uc *SubscribeMemberUseCase) Execute(ctx context.Context, req SubscribeMemberRequest) (SubscribeMemberResponse, error) {
	...
	if req.MemberID == "" {
		err := errors2.ValidationRequired("member_id")
	...
	if err != nil {
		if errors2.Is(err, store.ErrorNotFound) {
			...
			return SubscribeMemberResponse{}, errors2.NotFoundWithID("member", req.MemberID)
		}
		logger.Error("failed to get member", zap.Error(err))
		return SubscribeMemberResponse{}, errors2.Database("database operation", err)
	}
	...
	if uc.hasActiveSubscription(memberEntity) {
		...
		return SubscribeMemberResponse{}, errors2.ErrSubscriptionActive.WithDetails("member_id", req.MemberID)
	}
...
func (uc *SubscribeMemberUseCase) hasActiveSubscription(m domain.Member) bool {
