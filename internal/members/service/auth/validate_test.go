...
func TestValidateTokenUseCase_Execute(t *testing.T) {
	...
	validTokenPair, _ := jwtService.GenerateTokenPair("member-123", "user@example.com", string(domain.RoleUser))
	...
	invalidTokenPair, _ := differentSecretService.GenerateTokenPair("member-123", "user@example.com", string(domain.RoleUser))
	...
	tests := []struct {
	...
	}{
		{
			...
			validateFunc: func(t *testing.T, resp ValidateTokenResponse) {
				...
				assert.Equal(t, "member-123", resp.Claims.MemberID)
			},
		},
		{
	...
	tests := []struct {
	...
	}{
		{
			...
			validateFunc: func(t *testing.T, m *domain.Member) {
				assert.NotNil(t, m)
				assert.Equal(t, "member-123", m.ID)
