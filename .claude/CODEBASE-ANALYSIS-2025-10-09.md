...
```go
// Package auth provides HTTP handlers for authentication and authorization.
//
...
//
// All endpoints follow patterns defined in the parent handlers package.
...
//   - DTOs: internal/adapters/http/dto/member.go
package auth
```
...
```go
// internal/adapters/http/handlers/auth/handler_test.go
...
**Issue:** `internal/adapters/http/dto/payment.go` is 507 lines
...
// GatewayHelper encapsulates common gateway interaction patterns.
type GatewayHelper struct {
	gateway PaymentGateway
}

// GetAuthToken retrieves and logs gateway authentication token.
func (h *GatewayHelper) GetAuthToken(ctx context.Context, logger *zap.Logger) (string, error) {
	token, err := h.gateway.GetAuthToken(ctx)
	if err != nil {
		logger.Error("gateway authentication failed", zap.Error(err))
		return "", fmt.Errorf("gateway auth: %w", err)
	}
	logger.Debug("gateway authenticated")
	return token, nil
}

// MapStatus converts gateway status string to domain payment status.
...
// HandleGatewayError standardizes gateway error handling.
func (h *GatewayHelper) HandleGatewayError(err error, logger *zap.Logger) error {
	logger.Error("gateway operation failed", zap.Error(err))
...
```bash
# Check current usage
grep -r "BaseRepository\[" internal/adapters/repository/postgres
```
...
```go
// internal/domain/payment/gateway.go (NEW FILE)

package payment

import "context"

// Gateway defines external payment gateway operations.
...
type Gateway interface {
	...
	// CheckStatus verifies payment status with the gateway.
	CheckStatus(ctx context.Context, invoiceID string) (*GatewayStatusResponse, error)

	// InitiatePayment creates a new payment transaction with the gateway.
...
| DTO | `internal/adapters/http/dto/book.go` |
...
//   - Handler: internal/adapters/http/handlers/book/crud.go (HTTP layer)
