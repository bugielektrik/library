...
1. **`internal/infrastructure/pkg/handlers/author/handler.go`**
   - Added `authorops` import
   - Changed: `Execute(ctx)` → `Execute(ctx, authorops.ListAuthorsRequest{})`

2. **`internal/infrastructure/pkg/handlers/member/handler.go`**
   - Changed: `Execute(ctx)` → `Execute(ctx, memberops.ListMembersRequest{})`

3. **`internal/infrastructure/pkg/handlers/member/handler_optimized.go`**
   - Changed: `Execute(ctx)` → `Execute(ctx, memberops.ListMembersRequest{})`

4. **`internal/infrastructure/pkg/handlers/member/handler_v2.go`**
   - Changed: `Execute(ctx)` → `Execute(ctx, memberops.ListMembersRequest{})`
...
//   - Repository: internal/infrastructure/pkg/repository/postgres/{entity}.go
//   - HTTP handler: internal/infrastructure/pkg/handler/{entity}/{action}.go
