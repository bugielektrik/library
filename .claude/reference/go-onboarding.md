...
import "myproject/internal/infrastructure/pkg/repository"
...
find internal/infrastructure/pkg/repository -name "*.go" -type f | head -5
```
...
```bash
touch internal/infrastructure/pkg/repository/postgres/book.go
```

**4. Add HTTP Handler (5 min)**
```bash
touch internal/infrastructure/pkg/handler/book.go
```
...
package book
import "myproject/internal/infrastructure/pkg/repository"  // WRONG!
...
```go
// ❌ NEVER: Business logic in handler
