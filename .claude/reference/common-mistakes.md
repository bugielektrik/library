...
import (
    "library-service/internal/infrastructure/pkg/repository/postgres" // ❌ WRONG!
...
func NewContainer(repos *Repositories, ...) *Container {
    // ❌ Creating infrastructure service here!
...
// internal/app/app.go
func New() (*App, error) {
    // ✅ Infrastructure service created during app bootstrap
...
func NewContainer(repos *Repositories, caches *Caches, authSvcs *AuthServices, ...) *Container {
    // ✅ Domain service created here (lightweight, no config)
    bookService := book.NewService()
    memberService := member.NewService()

    return &Container{
        // ✅ Use infrastructure service from authSvcs
...
**Wrong:**
```go
// internal/infrastructure/pkg/handler/payment_page.go
...
// internal/infrastructure/pkg/handler/base.go (or use httputil.RespondJSON)
...
**Wrong:**
```go
// internal/infrastructure/pkg/handler/reservation.go
...
- Validation: internal/infrastructure/pkg/middleware/validator.go
